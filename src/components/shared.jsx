...
// ─── Product Card with Reusable WishlistHeart ─────────────────────────────────
export function ProductCard({ product, navigate, type = "default", layout = "grid" }) {
  const [addedMsg, setAddedMsg] = useState(false);
  const { addToCart } = useCart();
  const variants = getProductVariants(product);
  const [selectedVariantName, setSelectedVariantName] = useState(() => {
  ...
  // Mobile "list" layout — one product per row: a narrow vertical (portrait)
  // image strip on the left, details on the right, swatches scroll horizontally
  // instead of wrapping since row height is limited.
  if (layout === "list") {
    return (
      <a
      ...
      >
        {/* Image — narrow vertical rectangle */}
        <div style={{ position: "relative", width: "32%", maxWidth: 130, flexShrink: 0, aspectRatio: "3 / 4", overflow: "hidden", background: "#fafaf8" }}>
          {product.tag && tc && (
            <div style={{ position: "absolute", top: 6, left: 6, zIndex: 3, background: tc.bg, color: tc.color, fontSize: 8, fontWeight: 900, letterSpacing: "0.1em", padding: "3px 7px", borderRadius: 999, fontFamily: ff }}>
              {product.tag}
            </div>
          )}
          {discount > 0 && !product.tag && (
            <div style={{ position: "absolute", top: 6, left: 6, zIndex: 3, background: BRAND, color: BRAND_TEXT, fontSize: 8, fontWeight: 900, letterSpacing: "0.06em", padding: "3px 7px", borderRadius: 999, fontFamily: ff }}>
              −{discount}%
            </div>
          )}
          <WishlistHeart productId={product.id} size="sm" placement="card" />
          <img
            src={`.${displayImage}`}
        ...
        {/* Info */}
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center", gap: 7, paddingRight: 10 }}>
          <div
            style={{
              fontFamily: ff,
              fontSize: 14,
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontFamily: ff, fontSize: 13, fontWeight: 600, color: "#3a3a3a", letterSpacing: "0.02em" }}>
              Rs.{Number(discountPrice).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} PKR
            </span>
            {discountPrice < price && (
              <span style={{ fontFamily: mono, fontSize: 11, color: "#b8b8b8", textDecoration: "line-through" }}>
          ...
          {variants.length >= 1 && (
            <div className="pc-hscroll" style={{ display: "flex", gap: 8, overflowX: "auto", maxWidth: "100%", paddingBottom: 4 }}>
              {variants.map((variant) => {
                const active =
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      padding: 0,
                      boxShadow: active ? `0 0 0 2px ${BRAND}` : "0 0 0 1px rgba(0,0,0,0.12)",
                      flexShrink: 0,
                    }}
                  />
                );
